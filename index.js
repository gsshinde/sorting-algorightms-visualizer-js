...
var w = Number(window.screen.availWidth);
console.log(w);
...
function createGrid() {
  ...
  for (let i = 0; i < barCount; i++) {
    ...
    bar.style.height = `${value * barHeight}px`;
...
async function merge(start, mid, end) {
  ...
  for (var i = start; i <= end; i++) {
    if (p > mid) {
      ...
      Number(blocks[p].childNodes[0].innerHTML) <
      Number(blocks[q].childNodes[0].innerHTML)
    ) {
      Arr[k++] = Number(blocks[p++].childNodes[0].innerHTML);
      blocks[p - 1].style.backgroundColor = "yellow";
    } else {
...
async function quickPartition(start, end) {
  ...
  blocks[partitionIndex].childNodes[0].innerHTML = Number(
    blocks[end].childNodes[0].innerHTML
  );
...
async function heapSort() {
  ...
  for (let j = blocks.length - 1; j > 0; j--) {
    ...
    blocks[j].childNodes[0].innerHTML = Number(
      blocks[0].childNodes[0].innerHTML
    );
    blocks[j].style.height = `${Number(blocks[0].childNodes[0].innerHTML) *
      4}px`;
...
async function MaxHeapify(n, i) {
  ...
  if (largest != i) {
    ...
    blocks[largest].childNodes[0].innerHTML = Number(
      blocks[i].childNodes[0].innerHTML
    );
    blocks[largest].style.height = `${Number(
      blocks[i].childNodes[0].innerHTML
    ) * 4}px`;
